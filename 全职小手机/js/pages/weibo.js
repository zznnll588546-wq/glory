...
async function maybeLinkToChatAfterGenerate({ userId, post, actorId, actorName }) {
  ...
  const intended = (await findOrCreatePrivateChat(userId, actorId)) || pickRandom(groups) || pickRandom(all);
  if (!intended) return;
...
export default async function render(container) {
  ...
  const trendHtml = `
    <div class="card-block" style="margin:8px 16px;">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <strong>热搜榜</strong>
        <span class="text-hint">${season}</span>
      </div>
      <div style="margin-top:8px;display:flex;flex-direction:column;gap:6px;">
        ${(meta.trending || []).slice(0, 8).map((x, i) => `<div style="display:flex;gap:8px;font-size:13px;"><span style="color:#ff7a45;">${i + 1}</span><span>${escapeHtml(x)}</span></div>`).join('') || '<div class="text-hint">暂无热搜，点击右上角“⚡”生成</div>'}
      </div>
      <div style="margin-top:10px;font-size:12px;color:#6f8cab;">${(meta.news || []).slice(0, 3).map((n) => `• ${escapeHtml(n)}`).join('<br/>')}</div>
    </div>
  `;
  ...
  container.innerHTML = `
    ...
    <div class="page-scroll weibo-feed">${trendHtml}${listHtml}</div>
  ...
  container.querySelector('.weibo-generate')?.addEventListener('click', async () => {
    ...
    const prompt = [
      `你是微博运营编辑。时间线:${season}`,
      `用户:${user?.name || '旅行者'} ${user?.bio || ''}`,
      `角色池:${refChars}`,
      '必须只输出1个JSON对象，不允许任何解释、前后缀、注释、markdown。',
      'JSON Schema:',
      '{"trending":[""],"news":[""],"posts":[{"authorId":"id","authorName":"name","content":"text","fans":12345}]}',
      '约束：trending=8条，news=3条，posts=4-8条；所有字符串必须双引号；不允许尾逗号；若字段未知用空字符串或0。',
      '内容要贴合时代背景和职业电竞圈生态。',
      'trending/news 每条只写纯话题词，不要带引号或逗号。',
    ].join('\n');
    ...
    try {
      ...
      for (const p of parsed.posts || []) {
        const post = {
          ...
          authorId: p.authorId || p.authorName || 'npc',
        ...
        meta.profiles[post.authorId] = { ...(meta.profiles[post.authorId] || {}), fans: post.fans };
  ...
  container.querySelectorAll('.weibo-post').forEach((postEl) => {
    ...
    postEl.querySelector('[data-act="comment"]')?.addEventListener('click', async (e) => {
      ...
      root.querySelector('.wb-comment-send')?.addEventListener('click', async () => {
        ...
        if (!text) return;
        post.commentList = [...(post.commentList || []), { author: user?.name || '旅行者', content: text, timestamp: Date.now() }];
